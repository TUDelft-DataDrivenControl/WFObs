...
mainFolder = 'D:\bmdoekemeijer\My Documents\SurfDrive\PhD\Dissemination\2017 Wind Energy\MATLAB_out';
data{end+1} = struct(...
    'name','OL',...
    'path',[mainFolder '/KFcomp_MEAScomp_2turb_alm_turb_sim/workspace.mat']);
data{end+1} = struct(...
    'name','EnKF, SCADA (Pwr)',...
    'path',[mainFolder '/Q001_MEAScomp_2turb_alm_turb_enkf_n50_Pwr/workspace.mat']);
data{end+1} = struct(...
    'name','EnKF, Upw. LiDAR',...
        'path',[mainFolder '/Q001_MEAScomp_2turb_alm_turb_enkf_n50_upwLidar/workspace.mat']);
   data{end+1} = struct(...
    'name','EnKF, Dw. LiDAR',...
        'path',[mainFolder '/Q001_MEAScomp_2turb_alm_turb_enkf_n50_dwLidar/workspace.mat']);
    
    % data{end+1} = struct(...
    %     'name','EnKF, Downw. LiDAR',...
    %     'path',[mainFolder '/KFcomp_MEAScomp_2turb_alm_turb_enkf_n40_dwLidar/workspace.mat']);
    % data{end+1} = struct(...
    %     'name','OL',...
    %     'path',[mainFolder '/apc_9turb_alm_turb_simPoorLmu/workspace.mat']);
    % data{end+1} = struct(...
    %     'name','EnKF (n=50)',...
    %     'path',[mainFolder '/apc_9turb_alm_turb_n50/workspace.mat']);
    % data{end+1} = struct(...
    %     'name','EnKF, low Q (n=50)',...
    %     'path',[mainFolder '/lowQ_littleUpdates/apc_9turb_alm_turb_n50/workspace.mat']);
    ...
    % data{end+1} = struct(...
    %     'name','ExKF',...
    %     'path',[mainFolder '/Q001_KFcomparison_2turb_alm_turb_exkf/workspace.mat']);
    % % data{end+1} = struct(...
    % %     'name','EnKF',...
    % %     'path',[mainFolder '/KFcomparison_MEAScomparison_2turb_alm_turb_enkf_n30_dwLidar/workspace.mat']);
    % data{end+1} = struct(...
    %     'name','UKF',...
    %     'path',[mainFolder '/Q001_KFcomparison_2turb_alm_turb_ukf/workspace.mat']);
