...
sourcepath      = 'WFSim\Data_SOWFA\NoPrecursor\2turb_50x25_lin';    % Specify location of SOWFA data (excluding backslash at the end)
datanroffset    = 00000;                                             % Numbering offset (first filenumber is datanroffset+1)
...
switch lower(strucObs.filtertype)
    ...
    case {'enkf'}
        strucObs.nrens   =   60;         % Ensemble size
        
        % Default state parameters
        strucObs.R_e     =   0.10;       % Standard dev. for measurement noise ensemble
        strucObs.Q_e.u   =   0.20;       % Standard dev. for process noise 'u' in m/s
        strucObs.Q_e.v   =   0.04;       % Standard dev. for process noise 'v' in m/s
        strucObs.W_0.u   =   5.00;       % Width (in m/s) of uniform dist. around opt. estimate for initial ensemble
        ...
        strucObs.tuneModel.lmu = struct(...
                        ...
                        'W_0',  2.00,...  % Width (in m/s) of uniform dist. around opt. estimate for initial ensemble
        ...
        strucObs.r_infl         =   1.01;       % Covariance inflation factor (typically 1.00-1.20, no inflation: 1)
        strucObs.f_locl         =   'gaspari';  % Localization method: 'off', 'gaspari' (Gaspari-Cohn 1999) or 'heaviside' (Heaviside step function: 0s or 1s)
        strucObs.l_locl         =   70;         % Gaspari-Cohn: typically sqrt(10/3)*L with L the cut-off length. Heaviside: cut-off length (m).
