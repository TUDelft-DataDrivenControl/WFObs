...
strucObs.se.enabled = true; % Estimate the model states (flow fields)?
    ...
    strucObs.se.Rk.P = 1e-3;  % Autocovariance for turbine power measurements [if strucObs.measPw == 1]
    strucObs.se.Rk.u = 0.01;  % Autocovariance for long. flow measurements  [if strucObs.measFlow == 1]
    strucObs.se.Rk.v = 0.01;  % Autocovariance for lat.  flow measurements  [if strucObs.measFlow == 1]
