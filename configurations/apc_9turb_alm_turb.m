...
strucObs.measFlow = false; % Use flow measurements (LIDAR) in estimates
    ...
    strucObs.sensorsPath = 'sensors_apc_9turb_alm'; % Flow measurement setup filename (see '/setup_sensors/sensors_layouts')
...
strucObs.se.enabled = true; % Estimate the model states (flow fields)?
    ...
    strucObs.se.Qk.u = 1e-4; % Autocovariance for long. flow process noise
...
strucObs.pe.enabled = true; % Estimate model parameters?
    ...
    strucObs.pe.P0   = [1e0]; % Initial state covariance(s) for model variable(s)
...
switch lower(strucObs.filtertype)    
    ...
    case {'enkf'} % Ensemble Kalman filter (EnKF) 
        strucObs.nrens  = 8;        % Ensemble size
